...
class Transcriber:
    ...
    def send_to_tts(self):
        ...
        if self.audio_buffer:
            text_to_speech = "".join(self.audio_buffer)

            # generate audio using Bark
            audio_array = generate_audio(text_to_speech, history_prompt="v2/en_speaker_6")

            audio_file = "transcription_output.wav"
            write_wav(audio_file, SAMPLE_RATE, audio_array)

            os.system(f"start {audio_file}" if os.name == 'nt' else f"aplay {audio_file}")
